...
")
;; ListOfNumber -> Number
;; produce the largest element
(check-expect (largest empty) 0)
(check-expect (largest (cons 42 (cons 60 empty))) 60)
(check-expect (largest (cons 93 (cons 60 empty))) 93)
  ...
  (cond [(empty? lon) 0]
        [else
         (if (> (first lon) (largest (rest lon)))
             (first lon)
             (largest (rest lon)))]))
;; !!! this is a nasty implementation
;; and yet, this is the same as the instructor's solution.
;; I want let.
